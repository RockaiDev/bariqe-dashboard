...
function DataTable({
  ...
  const validateDateFilter = (
    dateFilterParam: DateFilter | null
  ): DateFilter | null => {
    if (!dateFilterParam) return null;
    if (
      (dateFilterParam.from && !dateFilterParam.to) ||
      (!dateFilterParam.from && dateFilterParam.to)
    ) {
  ...
  const applyFilters = () => {
    ...
    if (
      ...
      Object.keys(tempFilters).some(
        (key) => tempFilters[key] && tempFilters[key] !== "all"
      )
    ) {
  ...
  const activeFiltersCount =
    ...
    ).length + (dateFilter ? 1 : 0);
  const hasActiveFilters = activeFiltersCount > 0;
  const hasChanges =
    JSON.stringify(tempFilters) !== JSON.stringify(activeFilters) ||
    JSON.stringify(tempDateFilter) !== JSON.stringify(dateFilter);

  const hasDateError =
    tempDateFilter &&
    ((tempDateFilter.from && !tempDateFilter.to) ||
      (!tempDateFilter.from && tempDateFilter.to) ||
      (tempDateFilter.from &&
        tempDateFilter.to &&
        tempDateFilter.from > tempDateFilter.to));
  ...
  const renderPaginationButtons = () => {
    ...
    if (startPage > 1) {
      ...
      if (startPage > 2)
        pages.push(
          <span key="ellipsis-start" className="px-2 text-gray-400 text-sm">
            ...
          </span>
        );
    }
    ...
    if (endPage < lastPage) {
      if (endPage < lastPage - 1)
        pages.push(
          <span key="ellipsis-end" className="px-2 text-gray-400 text-sm">
            ...
          </span>
        );
      pages.push(
    ...
    <div className="relative max-w-full rounded-lg border shadow-sm bg-white mx-auto">
      ...
      <div className="flex flex-col md:flex-row gap-3 md:gap-2 justify-between items-start md:items-center p-4 border-b bg-gray-50">
        ...
        <div className="flex flex-wrap gap-2 w-full md:w-auto justify-start md:justify-end">
          ...
          {searchAble && (
            <div
              className="flex items-center bg-white border border-gray-300 rounded-md px-2 min-w-0 flex-1 md:flex-initial md:w-64 relative"
            ...
            >
              <PopoverTrigger asChild>
                <Button
                  ...
                  className={cn(
                    "gap-2 text-sm font-normal relative",
                ...
                <div className="p-4 space-y-4 max-h-96 overflow-y-auto">
                  <div className="flex items-center justify-between">
                    <h4 className="font-medium text-sm text-gray-700">
                      {intl.formatMessage({ id: "table.filter_options" })}
                    </h4>
                    <Button
                      variant="ghost"
                      size="sm"
                    ...
                    >
                      {intl.formatMessage({ id: "table.clear_all" })}
                    </Button>
                  </div>
                    ...
                    >
                      <div className="flex items-center gap-2">
                        <Label className="text-sm font-medium text-gray-700">
                          {intl.formatMessage({ id: "table.date_range" })}
                        </Label>
                        {hasDateError && (
                      ...
                      <div className="space-y-2">
                        <div className="space-y-1">
                          ...
                          <Input
                            ...
                            value={
                              tempDateFilter?.from
                                ? formatDateForInput(tempDateFilter.from)
                                : ""
                            }
                            onChange={(e) => {
                              const fromDate = e.target.value
                                ...
                                : undefined;
                              handleTempDateChange("from", fromDate);
                            }}
                            className={cn(
                              "w-full h-8",
                              hasDateError &&
                        ...
                        <div className="space-y-1">
                          ...
                          <Input
                            ...
                            value={
                              tempDateFilter?.to
                                ? formatDateForInput(tempDateFilter.to)
                                : ""
                            }
                            onChange={(e) => {
                              const toDate = e.target.value
                                ? new Date(e.target.value)
                                : undefined;
                              handleTempDateChange("to", toDate);
                            }}
                            className={cn(
                              "w-full h-8",
                              hasDateError &&
                          ...
                          !hasDateError && (
                            <div className="p-2 bg-blue-50 rounded-md border border-blue-200">
                              <p className="text-xs text-blue-800">
                                ...
                                {formatDate(tempDateFilter.from)} -{" "}
                                {formatDate(tempDateFilter.to)}
                              </p>
                            </div>
                          )}
                      </div>
                    ...
                    return (
                      <div key={group.key} className="space-y-2">
                        ...
                        <Select
                          value={selectedValue}
                          onValueChange={(value) =>
                            handleTempFilterChange(group.key, value)
                          }
                        >
        ...
        <div className="px-4 py-2 bg-blue-50 border-b flex flex-wrap gap-2 items-center">
          ...
          {Object.entries(activeFilters).map(([key, value]) => {
            ...
            return (
              <div
                key={key}
                className="flex items-center gap-1 px-2 py-1 bg-blue-100 rounded-md text-sm"
              >
          ...
          {dateFilter && (
            <div className="flex items-center gap-1 px-2 py-1 bg-green-100 rounded-md text-sm">
              ...
              <span className="text-green-800 font-medium">
                Date: {formatDate(dateFilter.from!)} -{" "}
                {formatDate(dateFilter.to!)}
              </span>
              <button
                onClick={clearDateFilterOnly}
  ...
  <div className="p-3 sm:p-4 flex flex-col gap-3 sm:gap-4 border-t bg-gray-50/50">
    {/* Mobile Layout */}
    <div className="flex sm:hidden flex-col gap-3">
      {/* Results Info - Mobile */}
      <div className="text-xs text-gray-600 text-center">
        <strong className="font-medium">{startItem}</strong>-
        <strong className="font-medium">{endItem}</strong>{" "}
        {intl.formatMessage({ id: "table.of" })}{" "}
        <strong className="font-medium">{pagination.total}</strong>
      </div>
      
      {/* Controls Row - Mobile */}
      <div className="flex items-center justify-between gap-2">
        {/* Per Page Input - Mobile */}
        <div className="flex items-center gap-1">
          <Label className="text-xs whitespace-nowrap">
            {intl.formatMessage({ id: "table.perPage" })}
          </Label>
          <Input
            type="number"
            min={1}
    ...
    <div className="hidden sm:flex items-center justify-between gap-4">
      ...
      <div className="text-sm text-gray-600">
        ...
        {intl.formatMessage({ id: "table.of" })}{" "}
        <strong className="font-medium">{pagination.total}</strong>{" "}
        {intl.formatMessage({ id: "table.results" })}
      </div>

      {/* Controls - Desktop */}
      <div className="flex items-center gap-2 lg:gap-3">
        {/* Per Page Input - Desktop */}
        <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-1">
          ...
          <Button
            variant="outline"
            size="sm"
            onClick={() => onPageChange && onPageChange(pagination.lastPage)}
            disabled={pagination.currentPage === pagination.lastPage}
            className="h-8 w-8 p-0 hidden lg:flex"
            title="Last page"
          >
